...
function contextualMFA(user, context, cb) {
  ...
  const completedMfa =
    ...
    !!context.authentication.methods.find(method => {
      if (method.name === 'mfa') {
        ...
        const requireMFAAt = method.timestamp + 100 * 1000;
  ...
  function isSensitiveOperation() {
    ...
    const sensitiveScopes = ['stepup:test', 'read:payment', 'update:payment'];
...
//  if (context.clientMetadata.userApp !== 'true') {
 ...
 // if (context.clientName === 'MTN Mobile') {
 //   callback(null, user, context);
 //   return;
 // }
  ...
  console.log(isSensitiveOperation());
